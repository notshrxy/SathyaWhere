/**
 * pages/Components/LP Comps/Profile-Preview.tsx
 * Advanced 'HoverPeek' component that displays user stats or link previews 
 * with a customizable magnifying lens effect.
 */
...
type HoverPeekProps = {
    ...
    stats?: UserStats; // New: Optional stats to display instead of just an image.
...
export const HoverPeek = ({
    ...
    enableLensEffect = false, // Lens is off by default when stats are used.
        ...
        >
            ...
            <RdxHoverCard.Portal>
                ...
                <RdxHoverCard.Content
                    ...
                    side={side} // Position card relative to the trigger.
                    align={align} // Align card relative to the trigger.
                        ...
                        {isPeeking && (
                            ...
                            >
                                {stats ? (
                                    ...
                                    <div className="flex bg-neutral-950 p-8 min-w-[620px] text-white rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden relative">
                                        {/* Avatar and Name Section */}
                                        <div className="w-[240px] shrink-0 flex flex-col items-center justify-center gap-6 relative z-10">
                                            <div className="relative w-44 aspect-square bg-white/5 rounded-[2.25rem] overflow-hidden border border-white/10 shadow-2xl">
                                                <div className="absolute inset-0 bg-violet-500/10 blur-[30px]" />
                                                <img
                                                    src={imageSrc || finalImageSrc}
                                                    className="w-full h-full object-contain relative z-10 p-4"
                                                    alt="User profile"
                                                />
                                            </div>
                                            <div className="text-center space-y-2.5 w-full">
                                                <h3 className="font-bold text-2xl leading-tight tracking-tight text-white">{children}</h3>
                                                {stats.regNo && (
                                                    <p className="text-[11px] text-white/40 font-mono tracking-wider">{stats.regNo}</p>
                                                )}
                                                <div className="pt-1">
                                                    <span className="px-3.5 py-1.5 bg-violet-500/10 border border-violet-500/20 rounded-full text-[10px] text-violet-400 font-bold uppercase tracking-[0.25em]">
                                                        {stats.rank}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="w-px bg-gradient-to-b from-transparent via-white/10 to-transparent self-stretch mx-12" />

                                        {/* Stats and Badges Section */}
                                        <div className="flex-1 flex flex-col gap-7 relative z-10 py-1">
                                            <div>
                                                <h4 className="text-base font-bold text-white/95 tracking-tight">Your Stats</h4>
                                                <p className="text-[11px] text-neutral-500">Active contributor since {stats.joinDate}</p>
                                            </div>

                                            <div className="grid grid-cols-2 gap-4 w-fit max-w-[280px]">
                                                {[
                                                    { label: 'RECOVERIES', val: stats.recoveries },
                                                    { label: 'REPORTS', val: stats.reports },
                                                    { label: 'RETURNS', val: stats.returns },
                                                    { label: 'RANK', val: stats.rank, color: 'text-violet-400' }
                                                ].map((s, i) => (
                                                    <div key={i} className="bg-white/5 p-4 rounded-2xl border border-white/5 transition-colors hover:bg-white/[0.07] flex flex-col justify-center w-[120px]">
                                                        <p className="text-[10px] font-bold text-neutral-500 tracking-[0.1em]">{s.label}</p>
                                                        <p className={cn("text-lg font-bold mt-1", s.color || "text-white")}>{s.val}</p>
                                                    </div>
                                                ))}
                                            </div>

                                            <div className="space-y-2">
                                                <h4 className="text-[11px] font-bold text-neutral-400 uppercase tracking-[0.2em]">Badges</h4>
                                                {stats.badges.length > 0 ? (
                                                    <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                                                        {stats.badges.map((b, i) => (
                                                            <div key={i} className="px-3 py-1 bg-violet-500/10 border border-violet-500/20 rounded-lg text-[10px] text-violet-300 font-semibold whitespace-nowrap">
                                                                {b}
                                                            </div>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    <p className="text-[10px] text-neutral-600 font-medium">No badges earned yet. Keep contributing!</p>
                                                )}
                                            </div>

                                            <div className="flex justify-between items-center mt-auto pt-3 border-t border-white/5">
                                                <span className="text-[11px] text-neutral-400 font-medium">Campus Standing</span>
                                                <span className="text-[11px] font-bold text-white bg-white/5 px-2 py-0.5 rounded-md">{stats.standing}</span>
                                            </div>
                                        </div>
