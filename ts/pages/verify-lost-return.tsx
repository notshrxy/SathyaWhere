/**
 * pages/verify-lost-return.tsx
 * Handover verification page for lost items.
 * Allows a user holding a lost item to verify the identity of the 
 * person claiming to be the owner before finalizing the return.
 */
...
const VerifyLostReturnPage = () => {
    ...
    useEffect(() => {
        if (id && user) {
            console.log('Fetching verification details for ID:', id, 'User ID:', user.id);
    ...
    const fetchVerificationDetails = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/lost-items/get-verification?id=${id}`, {
                headers: {
            ...
            if (!response.ok) {
                console.error('Verification fetch error:', data.error);
    ...
    const handleConfirmReturn = async () => {
        ...
        try {
            const response = await fetch('/api/lost-items/complete-return', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                setToast({
                    ...
                    message: 'Thank you for returning the item! Your stats have been updated.',
                    type: 'success'
                });
                ...
                >
                    <div className="bg-neutral-900/50 border border-white/10 rounded-[2.5rem] overflow-hidden backdrop-blur-xl shadow-2xl">
                        ...
                        <div className="p-8 pb-4 text-center">
                            <h1 className="text-3xl font-bold text-white">Handover Verification</h1>
                            <p className="text-neutral-400 mt-2">Verify the identity of the person receiving the item.</p>
                        </div>
                            ...
                            <div className="bg-white/5 border border-white/5 rounded-3xl p-6 space-y-6">
                                <div className="flex items-center gap-6">
                                    <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-violet-500/30">
                                        <img
                                            src={verificationData.owner.avatar_url || `https://api.dicebear.com/9.x/lorelei/svg?seed=${verificationData.owner.full_name}`}
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-xl font-bold text-white tracking-tight">{verificationData.owner.full_name}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-xl font-bold text-white tracking-tight font-mono">{verificationData.owner.registration_number}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pt-2">
                                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex items-center justify-center text-center">
                                    <p className="text-sm text-emerald-100/80 leading-relaxed font-medium">
                                        You are currently holding this item. Please confirm handover only after verifying the owner's ID and ensuring they are the rightful recipient.
                                    </p>
                                </div>
                            </div>
...
export default VerifyLostReturnPage;
