/**
 * pages/Components/LP Comps/Shuffle.tsx
 * Advanced text shuffle and scramble component with multiple directions, 
 * loops, and color transition support.
 */
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  loopDelay = 0,
  stagger = 0.03,
  scrambleCharset = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()_+-=[]{}|;:,.<>?',
  colorFrom,
  colorTo,
  triggerOnce = true,
  ...
  const autoTriggerIntervalRef = useRef<NodeJS.Timeout | null>(null);

  useEffect(() => {
    let isMounted = true;
    const checkFont = async () => {
      if ('fonts' in document) {
        // Specifically check for our pixel font if possible,
        // but document.fonts.ready is still the standard.
        try {
          await document.fonts.ready;
        } catch (e) {
          console.warn('Font loading promise failed, continuing anyway');
        }
      }
    ...
    checkFont();
    // Absolute fallback to ensure text isn't hidden forever
  ...
  useGSAP(
    () => {
        ...
        const addTween = (targets: HTMLElement[], at: number) => {
          tl.to(
            targets,
            {
              ...
              y: (i, t: HTMLElement) => parseFloat(t.getAttribute('data-final-y') || '0'),
              duration,
              ease,
              force3D: true,
      ...
      const create = () => {
        triggerAnimation();
        armHover();
        // Ensure we mark as ready
        setReady(true);

        // Set up auto-trigger if interval is specified
        if (autoTriggerInterval && autoTriggerInterval > 0) {
          const startAutoTrigger = () => {
            if (autoTriggerIntervalRef.current) {
          ...
          setTimeout(startAutoTrigger, duration * 1000 + 100);
        }
      };
  ...
  const commonStyle: React.CSSProperties = {
    ...
    fontSize: style.fontSize || '4rem',
    ...style
  };

  const classes = `${baseTw} ${ready ? '!opacity-100' : '!opacity-0'} transition-opacity duration-300 ${className}`.trim();
